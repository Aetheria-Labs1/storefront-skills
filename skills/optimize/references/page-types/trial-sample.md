...
6. `lexsis_design.islands`, then `lexsis_design.island_schema` for each supported island named below. A fit, shade, or size finder is deferred until a dedicated live island schema exists.
...
| `product-finder` or `size-guide` | a short quiz or sizing tool that changes which sample ships. | A size guide is an HTML table, or an image only when the merchant's chart cannot be transcribed. Finder media remains planned for a future dedicated island. | Use SizeGuide only when the merchant supplies measurements. Do not implement the finder until a dedicated live island schema supports its questions, result mapping, and BuyBox synchronization. | questions 12 words; options 4 words; "why we ask" 16 words. | fit, shade or size risk named in the brief; a supported schema is required. |
