...
| `quiz` (conditional: over 50 SKUs or meaningful recipient routing) | a gift-finder entry, usually three questions or fewer. | yes; one `context` image beside the entry from library `lifestyle`. Generation: none. Missing:; if skipped, use text choices. | use `Quiz` with deterministic branches and gift-result mappings; read `references/quiz-authoring.md` | one line and one CTA "Find a gift". | SKU count and approved result mappings. |
