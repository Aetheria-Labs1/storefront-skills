...
| `quiz` (conditional: over 50 SKUs and a routing quiz exists) | a gift-finder entry, three questions or fewer. | yes; one `context` image beside the entry from library `lifestyle`. Generation: none. Missing:; if skipped, the entry is a single button inside the `qualifier` chips. | omit until a dedicated live quiz island exists; a static CTA may link to an approved external finder | one line and one CTA "Find a gift". | SKU count and an approved destination. |
