...
### Section by section
**`announcement`** (conditional: warm or solution-aware traffic with a ledgered offer)
- Purpose: one line stating the offer or per-day price and the free-shipping threshold; the warm-traffic link.
- Media: no. No-go: a timer, a stock count, two bars.
- Island: `AnnouncementBar` with one static message for warm traffic, none for cold problem-led lists; resolve variant and props from `lexsis_design.island_schema`; preset `announcementbar/static-dark`.
- Copy: one line, under 60 characters, the offer exactly as the ad states it (`references/offers/offer-types.md`).
- Decide with: the offer ledger row; awareness from `lexsis_campaigns.analyze`.

**`hero`**
- Purpose: the numbered H1 with the audience or a verified count, a one-line subhead or verbatim quote, over an editorial header image.
- Media: yes; `editorial-lifestyle` echoing the ad, or `product-in-context`; the headline stays HTML text over a quiet zone. Search: the ad frame via `lexsis_campaigns.frames` (import the brand-owned original with `lexsis_asset_import.import`; a creator frame needs a rights row; use the clean original, since the exported ad usually carries baked-in text), then library `hero` then `lifestyle`, then semantic "<product> in use, <ad scene>", then merchant upload. Nothing found: ask rule; feasible purpose `hero_bg` under a real cut-out only when the plan names the header as the bold moment; fast-draft uses the best in-use image and lists the hero as missing. No-go: a grid of thumbnails, a packshot on white for cold social, a generated scene with people. View every candidate with `lexsis_assets.view` and run the fit review in `references/workflows/section-asset-workflow.md` section 1b (subject, crop, quiet zone for the copy, consistency with neighbouring slots, palette, no baked-in text or watermark) before it is used. Message match is judged by eye against the creative: same SKU, variant, angle and colourway, never by filename. `lexsis_asset_library.search` with `mode: "ocr"` flags library candidates that carry baked-in text before they are viewed.
- Island: `none`; a static `<picture>`. `HeroMedia` only for the full-bleed bold moment, autoplay and effects off unless the plan names that motion moment (N10); resolve variant and props from `lexsis_design.island_schema`.
- Copy: H1 at most 12 words, the number equal to the ad's number (`references/copy/message-match.md`); proof count rounded down with an as-of date, or the audience when reviews are below band B2; no blacklist words (`references/anti-patterns/copy-anti-patterns.md`).
- Decide with: `lexsis_campaigns.analyze`; `reviews_status` band; a `customer-count` ledger row.

**`stats`** or **`review-summary`**
- Purpose: the proof strip under the hero: rating with count, units sold, an expert credential.
- Media: no photography; an award or certification mark only as issuer artwork with a ledger row (`references/proof/press-and-media-mentions.md` for press). No-go: count-up animations, icon tiles, three conflicting counts. Any logo or mark is still opened with `lexsis_assets.view` before use.
- Island: `none`; HTML numbers (StatCards is deprecated). An average appears only at band B2 or higher, always beside n.
- Copy: two or three numbers, each with its unit and as-of date where older than 90 days.
- Decide with: ledger rows for every numeral; RS3 in `references/proof/reviews-sourcing.md`.

**`comparison`**
- Purpose: a category-reframing table or quick-stats bar above the first reason.
...
- Copy: four to six rows, cells at most 6 words, one concession row.
- Decide with: `test-data` rows per cell.

**`list-item`** (five to seven, each its own section)
- Purpose: one objection or desire answered per reason, each with one image and a benefit caption.
- Media: yes, one real image per reason, job chosen by the reason's role: reason 1 (the category's biggest objection) takes `detail` or `scale`; the mechanism reason takes `ingredient-or-material`, `sequence` or an authored `diagram`; the fear or tension reason takes `result-or-context` (a context image of the situation; a result only with a verified `P` row); the identity reason takes `in-use` or rights-cleared `ugc`; the closing reason takes `included-items` or `packaging` for an offer, `founder-or-team` for a founder story, or the `identity` shot beside the guarantee. Search per reason: catalog media by job, then library `product-shot`, `lifestyle`, `flat-lay` or `social-proof`, then semantic "<reason claim>", then merchant upload. Nothing found for a reason: ask rule naming the reason, its job and aspect; feasible purpose `product_composite` for one `context` reason at most, never `in-use`, `scale`, results or ingredients (GP14); the same image at a second crop serves at most one extra reason; shortening the list or re-typing the brief as `ad-landing-page` happens only on the merchant's choice and is recorded. No-go: emoji or icon markers, a caption baked into the image, a reason that is a feature without an outcome. View every candidate with `lexsis_assets.view` and run the fit review in `references/workflows/section-asset-workflow.md` section 1b (subject, crop, quiet zone for the copy, consistency with neighbouring slots, palette, no baked-in text or watermark) before it is used. View all reason images together and judge the set: one shoot, one lighting run, one crop scale, not a pile of found images. `lexsis_asset_library.search` with `mode: "similar"` and `similar_to_asset_id` set to the first accepted reason image pulls candidates that match its lighting and styling; that mode ignores `query`, `kind` and `theme_id` and takes only `limit` (at most 48), so it returns the seed asset's neighbours rather than a filtered set, and each one still goes through the fit review.
- Island: `none`. A matched per-reason CTA is an HTML anchor to the buy block, soft copy first.
- Copy: `listicle` spine; each reason a mini `fab` (the fear reason a mini `pas`); subhead at most 8 words; body at most 60 words; a number, material, time, test or named person in every reason; a quote or citation inside at least two reasons; no blacklist words (`references/anti-patterns/copy-anti-patterns.md`).
- Decide with: the image inventory from context reads 1 and 5; `reviews_search` per reason; `test-data` rows.

**`reviews`**
- Purpose: three to six titled, named, dated quotes after the list.
- Media: `ugc` screenshots or `review-with-media` where rights exist (`has_media: true`; library `social-proof` with a `P` row). Nothing found: the quotes run as text; generation is never feasible (GN9). View every candidate with `lexsis_assets.view` and run the fit review in `references/workflows/section-asset-workflow.md` section 1b (subject, crop, quiet zone for the copy, consistency with neighbouring slots, palette, no baked-in text or watermark) before it is used.
- Island: by band per `references/proof/reviews-sourcing.md`. B1: one quote under the reason it matches, static. B2 and above: static quote blocks matched to reasons plus an average and n at the close, or `ReviewCarousel` bound to an active collection when the quotes do not map to reasons, all cards visible, autoplay off (its default is on, N10); resolve variant and props from `lexsis_design.island_schema`; preset `reviewcarousel/grid-flat`. Never first. B0: `guarantee`, `certifications` and product evidence replace it; the proof-count headline becomes an audience headline; the omission is recorded.
- Copy: verbatim, dated, attributed as stored, titled where the data has titles; at most 60 words.
- Decide with: `reviews_status`; active `review_collections`; `reviews_search` hits confirmed by the merchant.

**`buy-box`** or **`closing-cta`**
- Purpose: the close: an embedded buy block on the same URL, or one CTA block to the PDP.
- Media: a gallery of three from catalog media (`identity`, `scale`, `variation`) for the embedded form; one identity image for the link form. Nothing found: ask rule; no generation is feasible in a gallery (GP11). View every candidate with `lexsis_assets.view` and run the fit review in `references/workflows/section-asset-workflow.md` section 1b (subject, crop, quiet zone for the copy, consistency with neighbouring slots, palette, no baked-in text or watermark) before it is used.
- Island: `BuyBox`, one per page, when the close is a full buy block with shipping line and guarantee, its form decided by the variant count and axes, with `ProductHero` for the three images; `QuickAdd` when the close is one button beside the identity image and the product needs a variant picker but no quantity or subscription; `none` (one link) when the merchant keeps the PDP as the buy surface. `StickyBar` bound to the buy block, appearing after reason 1 scrolls out, the offer or per-day figure in its label; it has no variant sync, so it carries a variant only for a single-variant embedded buy box and otherwise links to the block. Resolve variant and props from `lexsis_design.island_schema`; presets `buybox/default-light`, `producthero/stacked-dots-quiet`, `stickybar/collection-light`.
- Copy: `add-to-cart` on the embedded button ("Add to cart, $39"); `next-step` on a link ("Get the pan"); savings math for a bundle close from the ledger (`references/offers/offer-types.md`); an open loop ("reason 11 revealed at checkout") optional.
- Decide with: variant count and axes; the merchant's buy surface; offer ledger rows; page length.

**`trust-bar`**
- Purpose: guarantee, shipping and certification facts under the close.
- Media: no photography; certification marks only as issuer artwork with a ledger row. Nothing found: ask rule; generation is not feasible for badges (GN5). Any logo or mark is still opened with `lexsis_assets.view` before use.
- Island: `none`; a static HTML row (Marquee is deprecated).
- Copy: three or four facts, at most six words each.
- Decide with: policy URLs and certification rows.

**`faq`** (conditional: three or more real objections not covered by a reason)
- Purpose: the leftovers, collapsed.
- Media: no.
- Island: `none`; native `<details>` and `<summary>` (the FAQ island is deprecated).
- Copy: answers at most 60 words.
- Decide with: `reviews_search` on objection topics not already used as reasons.
