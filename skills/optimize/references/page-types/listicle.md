...
2. Read the supplied campaign material: record the number in its headline, the noun phrase and promise, the audience, and the scene for the editorial header (`references/copy/message-match.md`). Select a real frame for video.
...
With only an identity shot and one in-use image the page cannot carry five imaged reasons: the plan lists the four missing reason images with the upload offer, a delegated brief ships hero, a two-reason list, reviews at any band above B0, the buy block and trust bar with the gaps recorded, and the list shortens or the brief is re-typed as `ad-landing-page` only on the merchant's decision. Generated assets: at most four per page, usually zero or one `hero_bg`.
