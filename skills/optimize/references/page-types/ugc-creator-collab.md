...
1. `lexsis_catalog.get` for the hero product and each pick: `identity` image per product, variant axes (the creator's variant is pre-selected where known; resolve variant imagery from the live purchase contract, so colour-with-image needs VariantSwatches), price and compare-at (the creator discount is a ledger row with the list price as basis, `references/offers/offer-types.md`), selling plans (ignored: subscribe-save is a misfit), inventory (a real drop cap only), `included-items` media for a kit.
2. Read the supplied original post or whitelisted ad: record the hook line the welcome repeats (`references/copy/message-match.md`), the offer in the caption, the real frame that becomes the hero poster, and paid or gifted status.
...
|---|---|---|---|---|---|
| `hero` | the creator's clip or portrait, their handle and disclosure label in frame, a one-line welcome in their words, the discount stated as a fact, one CTA. | yes; `ugc` as `video-poster` from the creator's 9:16 clip (a real frame with the creator and the product), or `ugc-screenshot` when the click came from a static post; 4:5 or 9:16. Search: the supplied sending post's original with a `P` row, then library `social-proof` with a `P` row. Nothing found: ask rule naming the rights row and the original file; generation is never feasible for people or UGC (GN3, GN9); the type is blocked until the merchant supplies it. No-go: a studio packshot, stock imagery, a second creator, autoplay with sound, urgency of any kind. While viewing the clip, check that the disclosure label is in frame and that the `P` row matches this file. Viewing confirms the creator matches the rights row, the product is identifiable in frame, the clip is natively vertical, and any burned-in caption is legible and does not repeat the headline. | `VideoPlayer` for the clip, click to play with the poster, captions supplied with the media object; a muted captioned loop of 6 to 15 seconds only when the plan names it as the single motion moment with a pause control (N10). The disclosure label and handle are HTML over the frame. The CTA is an anchor to the spotlight. | welcome at most 25 words, verbatim and approved by the creator; "Your 15% is applied at checkout" with the free-shipping threshold; "Paid partnership" or "Ad" exactly as the market permits; no "obsessed" or "must-have" unless the creator said it (`references/anti-patterns/copy-anti-patterns.md`). | the `P` row status; supplied hook and offer; the offer ledger row. |
| `product-spotlight` | the hero product with price, variant, shipping and returns line, the creator's quote about it, add-to-cart. | yes; the `identity` image from catalog media (the one place a brand packshot belongs), plus one creator still of the product in use where rights exist. Gap for identity: shared fallback; no generation is feasible (GP11). | `BuyBox`, one per page, its form decided by the variant count and axes, badges off, the applied discount in its label; `VariantSwatches` when colour variants carry images and to pre-select the creator's variant. `DeliveryEstimate` for the dispatch cutoff when `shipping-returns` is not a separate section (India pincode copy is static HTML). `StickyBar` bound to the spotlight, appearing after the hero CTA scrolls out with the discount fact in its label; resolve shared purchase state with the spotlight form. | `add-to-cart` ("Add to cart, 15% applied"); list price struck only with the ledger basis; the creator's quote at most 30 words, verbatim. | variant axes and images; the percent-off or fixed-off ledger row and its auto-apply mechanic (never a code to type). |
| `ugc-grid` | the post that sent the visitor plus three to six more creator clips or stills, handle on each, disclosure for the whole brand segment. | yes; `ugc` clips at 9:16 native with captions and real poster frames, or stills at 1:1 or 4:5, uniform per grid, crop only, original grading; each item with its own `P` row and music cleared. Search: library `social-proof` with `kind: "video"` and `P` rows, then `lexsis_asset_import.import` of the creator's originals with consent. Fewer than four items: ask rule naming the count and the rights record each item needs; generation is never feasible (GN9); a smaller grid or none as the agreed alternative. No-go: brand-shot "UGC-style" content labelled as creator content, platform-licensed music kept in a hosted clip, a carousel. View the tiles together as one set and check the disclosure label and the `P` row for each while viewing. Viewing confirms the creator matches the rights row, the product is identifiable in frame, the clip is natively vertical, and any burned-in caption is legible and does not repeat the headline. | `ShoppableVideoFeed` for three or more clips, posters on every item, click to play, sound off until tapped, creator name and caption shown, the hero product tagged; native `<video>` tiles for one or two clips; a static grid for stills. | handle visible on each tile; the disclosure label in frame; captions on video. | clip count and `P` rows; the UGC row in `references/assets/video-rules.md` (20 to 40 seconds, 9:16). |
| `shop-the-look` (conditional: the creator picked three to six products) | one card per pick with one line in the creator's words. | yes; one catalog `identity` image per pick via `lexsis_catalog.get`; a creator still with the pick in use where rights exist. Gap: no generation is feasible. Viewing confirms the creator matches the rights row, the product is identifiable in frame, the clip is natively vertical, and any burned-in caption is legible and does not repeat the headline. | `QuickAdd` on at most two cards (the CTA ceiling), links to the PDPs on the rest; alternatively the picks are tagged inside the `ShoppableVideoFeed` items. No `ProductCarousel` below four picks, and its entry animation off if used (N10). | `fab` on the card; the creator's sentence at most 30 words, verbatim; "Shop <name>'s picks" only for this variant. | pick count; the CTA ceiling of 2 to 3. |
| `routine` or `usage` | how the creator uses it: steps, time of day, frequency, in their words. | yes; `sequence` of three to five creator stills or one creator clip showing the routine, each with a `P` row. Search: library `social-proof`, then the creator's originals imported with consent. Gap: generation is never feasible; the routine collapses into the hero clip's caption only if the merchant chooses. Check the `P` row for each still or clip while viewing. Viewing confirms the creator matches the rights row, the product is identifiable in frame, the clip is natively vertical, and any burned-in caption is legible and does not repeat the headline. | `none` for stills; `VideoPlayer` for one routine clip, click to play | `bab` in the creator's voice; at most 120 words; step text in HTML. | what the creator actually filmed; `P` rows. |
| `reviews` or `review-summary` | the mandatory non-creator proof beside the creator's voice. | `review-with-media` cards from `reviews` with `has_media: true`; avatars real or CSS initials, never generated. | by band per `references/proof/reviews-sourcing.md`. B0 and B1: no review module; `guarantee` plus `certification` supply the non-creator proof and the plan records it. B2: `ReviewCarousel` bound to the product id showing three to six media reviews, autoplay off (its default is on, N10). B3 and B4: a media grid of six to twelve, or `ReviewList` with media on. Creator content is never mixed in. | verbatim, dated, attributed as stored; the rating with n only at B2 or higher. | `reviews_status` band; `has_media` count. |
| `offer` (conditional: a creator kit or bundle exists) | the kit with savings math from the ledger, never a code. | yes; `included-items` flat lay of every shipped component from catalog media or merchant upload. Gap: generation is never feasible for contents (`image-jobs-by-page-type.md` section 7, GP13); the kit ships as a list with component identity images meanwhile. | when the kit is the hero product the spotlight `BuyBox` sells it; otherwise a card with `QuickAdd` or a link, since the page carries one BuyBox | "$87 separately, $59 as a set, save $28" from the ledger (`references/offers/offer-types.md`); "Add <name>'s kit". | the bundle ledger row and component prices from `lexsis_catalog.get`. |
| `faq` | shipping, returns, "is this the same product <name> uses", how the discount applies. | no. | `none`; native `<details>` and `<summary>`. | answers at most 60 words; second person for brand policy lines. | policy URLs; the discount mechanic in the ledger. |
| `shipping-returns` (recommended when not already in the spotlight) | the policy facts in full. | no. | `DeliveryEstimate` for the cutoff line when shipping is domestic and predictable, `none` otherwise | days, scope and friction stated; policy URL. | `lexsis_brand.context` policy facts. |
| `closing-cta` | the same action; the creator's sign-off line above it. | optional reuse of the hero poster or spotlight identity slot; no new job. | `none`; the CTA anchors to the spotlight. | the creator's sign-off verbatim, at most 25 words; the same CTA text as the spotlight. | the creator-approved copy record. |
...
| supplied campaign material | the original whitelisted post: hero poster, hook line, stated offer | anything the creator did not film | choose a real poster frame from the supplied original; the creator's consent must name the website, not only ads |
