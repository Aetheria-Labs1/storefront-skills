...
recommendation is a form and routes to `lead-capture-giveaway`. Implement the
...
7. `lexsis_cart.get`: record the intended effective Cart V2 behavior. Before design, query `lexsis_design.islands` and read the current `Quiz` schema.
