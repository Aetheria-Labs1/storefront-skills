...
7. `lexsis_design.islands`, then `lexsis_design.island_schema` for `EmailCapture`; deprecated entries take their replacement per `references/workflows/island-selection-workflow.md`.
...
| `referral-form` | both sides of the reward in currency, email share first. | no. | `EmailCapture` when the share is a supported single friend-email field; otherwise link to the approved referral app portal. Resolve supported props from `lexsis_design.island_schema`; the reward maths live in HTML beside the form, not in a discount line. Copy-link and social sharing are provided only by the approved referral system, not improvised page JavaScript. | 25 words plus the terms line (friend's minimum spend, expiry). | the schema in read 4; no supported schema and no portal URL means the section waits. |
