...
| `faq` | four to six objections, shipping, returns and "who it is not for" always present. | no. No-go: card-wrapped questions (N8). | `none`; native `<details>` and `<summary>`. | objection-phrased questions; answers at most 60 words. | `reviews_search` on objection topics; ad comments surfaced by `lexsis_campaigns.analyze`. |
