...
7. `lexsis_design.islands`, then `lexsis_design.island_schema` for `EmailCapture` and, only with a verified end datetime, `CountdownTimer`. `Modal` is read only for pages that link here; it never mounts on this page.
...
| `giveaway-entry`, `email-capture` or `sms-capture` | the one form and the one CTA of the page. | no. The form is the object; it sits directly under the hero image with its consent text and button visible without scrolling. | `EmailCapture` supports a single email field. Multi-field, phone, or mixed-channel capture must link to an approved provider until a dedicated live island schema exists. Resolve supported props from `lexsis_design.island_schema`; no discount line on the page (the code travels by message); the unticked consent checkbox with full disclosure is authored in HTML beneath the island, never `required`, with separate consent for email and SMS (DP5). No `Modal` on this page; on pages that link here, `Modal` as exit intent only, once per session, with `EmailCapture` as its only child. | button names the delivery; consent text in full for the market; confirmation state names the channel and the wait. | the schema in read 2; the consent template for the market from read 6. |
