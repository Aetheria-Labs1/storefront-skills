...
| `quiz` (conditional: choice complexity and a routing quiz) | finder entry with one CTA. | yes; one `context` image beside the entry from library `lifestyle`. Missing:; if skipped, the entry is the hero's secondary CTA. | omit until a dedicated live quiz island exists; do not invent a runtime or hidden control | one line and one CTA. | an approved destination implemented by a supported island or external provider. |
