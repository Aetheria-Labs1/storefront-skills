...
| `quiz` (conditional: choice complexity and a routing quiz) | finder entry with one CTA. | yes; one `context` image beside the entry from library `lifestyle`. Missing:; if skipped, the entry is the hero's secondary CTA. | use `Quiz` inline or link to an approved Quiz page; do not invent a second runtime or hidden control | one line and one CTA. | an approved Quiz definition and destination. |
