...
| `gift-options` | note, un-ticked priced wrap, gift receipt wording, gift returns. | yes; job `gift-presentation` (the real box, wrap and card that ship). Search catalog media of the wrap SKU, then library `product-shot` and `lifestyle`, then semantic "<brand> gift box", then merchant upload. Generation: none (identity-bound; a composite may not add wrap, GP13). Missing media: (a photo of what ships, 1:1 or 4:5, one image) and; if skipped, the note and wrap controls stay as the section's object and the plan waits for the photo (`gift-presentation` is required for this type). | note field and wrap add-on come from the effective cart profile (`lexsis_cart.get`); QuickAdd on the wrap SKU when wrap is a product Nothing pre-ticked (CC12). | label with the character limit, wrap price beside the control, "Gift receipt hides prices", returns window; four lines. | cart profile flags and the policy URL. |
...
| `faq` (recommended) | cutoffs by speed and destination, gift returns, wrap and note, the packing slip. | no. | FAQ island in single-open mode. | four to six questions, answer in the first sentence, 60 words each. | support data and the ledger rows. |
