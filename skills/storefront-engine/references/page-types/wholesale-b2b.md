...
8. `lexsis_design.islands`, then `lexsis_design.island_schema` for supported capture islands; the logo row and any region tabs are native HTML per `references/workflows/island-selection-workflow.md`.
...
| `email-capture-inquiry` | the wholesale application. | no. | A single-field `EmailCapture` may request a sales follow-up. A full multi-field application must link to an approved provider until a dedicated live application island supports country logic and file upload. Do not simulate steps in page JavaScript. The B2B consent line is unticked and never `required`; no consumer newsletter box. | labels above fields with required and optional marked; one line on why the tax identifier is asked; button "Apply for a wholesale account". | the schema fields and country logic from read 5, plus an approved provider URL when more than email is required. |
