...
recommendation is a form and routes to `lead-capture-giveaway`. This release
supports planning the journey only. Design must stop until a dedicated live
quiz island schema supports questions, branching, results, and cart handoff.
...
1. Record the quiz shape in the approved plan: step types, branching, whether options carry images, how results map to products, and how the result hands off to cart.
...
7. `lexsis_cart.get`: record the intended effective Cart V2 behavior. Before design, query `lexsis_design.islands`; if no dedicated quiz island is present, return `DESIGN_BLOCKED_UNSUPPORTED_ISLAND` and do not author a hidden or custom runtime.
...
| `quiz` | 5 to 8 questions, one per screen, image choice where possible, every answer mapped to a product or collection, skip logic, progress, no open text. | plan real option imagery from catalog variant media or approved library scenes; never stock or generated people as "you". | blocked until a dedicated live quiz island schema exists; do not implement with section JavaScript, hidden controls, or undocumented events | questions 12 words; options 4 words; "why we ask" 16 words; second person. | approved step and result configuration from step 1; option imagery found in steps 2 and 6. |
| `email-capture` | after the last question and before results; skippable; consent un-ticked. | none. | part of the future quiz island, or a supported EmailCapture after results; never a hard gate without merchant confirmation | "we will send these results to you" (12 words) and the skip link in plain words. | lead goal in the brief. |
| `quiz-results` | personalised header using the answers, a summary of what was learned, the reason for the result. | the first recommended product's identity from catalog media leads the screen; never a generated result image (GN4). | blocked with the quiz until the dedicated island schema defines result mapping | the header repeats the shopper's words; "results", never "diagnosis" without a clinician and disclaimer. | approved result mapping from step 1. |
