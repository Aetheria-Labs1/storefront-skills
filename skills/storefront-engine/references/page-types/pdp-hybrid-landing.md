...
1. `lexsis_catalog.get` with the product id: every media item mapped to a job (`identity`, `detail`, `scale`, `in-use`, `variation`, `included-items`, `label-or-facts-panel`), the current order, per-variant image mapping (a `variation` slot per variant that lacks one), variant axes and count (identify which variant images need coordinated selection), price and compare-at (ledger basis), selling plans (a subscribe-save toggle only for consumables with cancellation terms, `references/offers/offer-types.md`), inventory (a stock line only from live data), and the exact title the feed uses.
2. Traffic source. For paid social, the supplied campaign message and frame give the H1 promise and the creative direction for gallery slot one (`references/copy/message-match.md`). For Google Shopping, the feed image, title, price and availability govern slot one and the H1 (`references/traffic-source-google.md`).
...
|---|---|---|---|---|---|
| `hero` | a 120 to 220px message-match band: H1 restates the ad promise, one subhead, `review-summary`; no CTA, no image of its own. | no. The first gallery image directly below is the hero image; a band image would push price and add-to-cart below y=1266 at 390px. Search-intent variant: the band collapses into the product title. | `none`. | H1 at most 10 words with 60% token overlap with the ad (`references/copy/message-match.md`); subhead one line; average plus n at band B2 or higher, "n reviews" at B1; no blacklist words (`references/anti-patterns/copy-anti-patterns.md`). | supplied campaign message for paid social; the feed title for Shopping traffic; the review band. |
| `gallery` | five to eight images that let the shopper judge identity, size, use, detail and their own variant. | yes; `identity` at slot one (the feed packshot for Shopping traffic; the ad creative at slot one and the packshot at slot two for meta traffic), then `scale`, `in-use` or on-model, `detail`, `variation` per selected variant, `label-or-facts-panel` or `ingredient-or-material` where the vertical needs it, a demo video at slot two or three with a real poster (`references/assets/video-rules.md`). Search: catalog media, then library `product-shot` and `lifestyle`, then semantic per job, then merchant or supplier upload. Nothing found below the vertical minimum: ask rule naming each missing job and count; no generation is feasible in the gallery (GP11, GN12); the gallery ships with verified images only, no placeholder. `lexsis_asset_library.search` with `mode: "ocr"` flags library candidates that carry baked-in text before they are viewed. | `ProductGallery`, or `ProductHero` in a split layout beside the buy box for fashion, furniture and premium goods where media should take 50 to 60% of desktop width; decide layout, thumbnails, mobile behaviour and lightbox from the image count, the vertical and whether colour variants carry images (variant sync on when VariantSwatches is present); autoplay and transitions off unless the plan names the motion moment (N10). | alt per slot from the slot template in `references/assets/slot-spec.md`; thumbnails all visible up to 10 to 14, then an explicit "+N". | image count against the vertical minimum; variant image mapping; traffic source. |
| `buy-box` | untruncated title, rating and count, price and compare-at, a two to four sentence story block, variant buttons with a fit sentence, quantity, add-to-cart, shipping and returns line, guarantee, payment options. | the gallery above supplies it; `swatch` images per colour variant from catalog media or library (flat CSS chips only beside a real swatch). Gap: no generation is feasible for swatches. | `BuyBox`, one per page; decide its form from the variant count and axes and whether the page has its own icon set. `VariantSwatches` when colour or shade variants carry images (the fit sentence per option is HTML beside it). `DeliveryEstimate` for the dispatch cutoff (India pincode copy is static HTML). `InventoryIndicator` only when bound to live inventory. `StickyBar` bound to the buy section when the page runs past three viewports; link it to the main purchase state using the live schema. | `bab` intro at most 60 words; "Add to cart" or "Add to bag" for fashion and beauty; per-serving or per-day line only when arithmetically true; subscribe-save copy shows renewal price and cancellation terms beside the toggle (`references/offers/offer-types.md`). | variant axes and images, selling plans, inventory, offer ledger rows, page length. |
| `trust-bar` | policy facts under the add-to-cart. | no photography; payment or certification marks only as issuer artwork with a ledger row (`references/proof/press-and-media-mentions.md` for any press). Gap: generation is not feasible for badges (GN5). Any logo or mark is still opened with `lexsis_assets.view` before use. | `none`; a static HTML row (Marquee is deprecated). | three or four facts, at most six words each. | policy URLs and certification rows in the ledger. |
| `benefits` | three to six outcomes with a number, material, time or test. | yes; one image per benefit with the product present (`detail`, `in-use`, `scale`, `included-items`), reusing gallery images at a different crop only once. Search: catalog media, then library `product-shot` and `lifestyle`, then semantic, then merchant upload. Gap for a benefit: shared fallback; feasible purpose `product_composite` for a `context` benefit only; a benefit merges only on the merchant's choice. No-go: icon tiles, emoji, generated in-use. | `none`. | `fab`; subhead at most 8 words; body at most 25 words. | the job map; one slot per benefit. |
| `mechanism` or `how-it-works` | why it works, with a diagram where one exists. | yes; `diagram` as authored inline SVG with numbers in HTML, or `sequence` of real step photos. Search: catalog media, then library, then merchant or supplier upload, then author the SVG. Gap: generation is not feasible for steps or a raster diagram; the authored SVG carries the section meanwhile. No-go: stock lab imagery, three icon tiles. | `none`; `VideoPlayer` only when a real 45 to 90 second demo exists and is not already in the gallery, click to play with a real poster | three steps, at most 25 words each; study n and design disclosed beside any statistic. | `test-data` ledger rows; catalog coverage of `sequence`. |
| `ingredients`, `specs` or `materials` (conditional: supplements, skincare, food, apparel, electronics) | what it is made of, as facts. | yes; `label-or-facts-panel` as a legible real photo (1600px or more) with the same figures in HTML, or `ingredient-or-material` flat lay, or a `detail` macro of fabric or ports. Search: catalog media, then library `flat-lay` and `product-shot`, then supplier upload with a licence. Gap: generation is not feasible (GN6, GN11); the panel is an HTML table meanwhile. | `none`; the table is HTML (Tabs is deprecated and horizontal tabs are forbidden on this type). | every number in HTML matches the label; supplier provenance only where verified. | the vertical row in `imagery-by-vertical.md`; ledger rows. |
| `before-after` (conditional: substantiated, consented, category permitted) | a genuine result pair for a results objection. | yes; a verified `before-after` ledger row with the same subject, crop and lighting, interval stated in HTML. Search: library tag `before-after` with a `P` row, then merchant upload with consent on file. Nothing found: ask rule, naming the consent and substantiation the pair needs; generation is never feasible (GN4); the section exists only with a verified row and never in the hero. | `BeforeAfter` when both images share one aspect ratio, static side-by-side otherwise | "Before" and "After" with the interval; "Individual results vary" where the category requires it. | `references/proof/before-after-and-claims.md`; the `P` row status. |
| `expert-endorsement` | a named person with a credential and disclosed connection. | yes; `founder-or-team` portrait of the named expert (real, consented) from the library or merchant upload. Gap: generation is never feasible for people (GN3); the quote runs with the credential line as text meanwhile. | `none`. | quote at most 60 words; name, credential, connection disclosed. | the `expert-quote` ledger row with written approval. |
| `comparison` | us versus the category, or the merchant's own tiers, right below the description. | one `comparison-visual` of our attribute physically; the alternative is an inline SVG silhouette. Search: catalog `detail` or `scale`, then library, then merchant upload. Gap: generation is not feasible (GN8); the table stands with our identity image in the header meanwhile. No-go: competitor photos, tick grids without a concession row. | `none`; a static HTML table. | four to six rows, cells at most 6 words. | `test-data` rows per cell. |
| `ugc-grid` | six to twelve rights-cleared customer tiles, uniform aspect, handles visible. | yes; `ugc` from `reviews` with `has_media: true` and library `social-proof`, each with a `P` row; crop only, original grading (`references/proof/ugc-rights-and-display.md`). Fewer than six: ask rule naming the count and the rights record each tile needs; generation is never feasible (GN9); a smaller grid or none as the agreed alternative. | `none` for a static grid of stills; `ShoppableVideoFeed` when the tiles are 9:16 clips with the product tagged, posters on every item | "Customer photos" label; first name or handle when permitted. | `has_media` count; UGC ledger rows. |
| `reviews` | the full module with distribution and filters, one critical review reachable. | photo reviews navigable (`review-with-media`); avatars real or CSS initials, never generated. | by band per `references/proof/reviews-sourcing.md`. B1: static cards. B2: `ReviewCarousel` bound to the product id or an active collection, autoplay off (its default is on, N10). B3 and B4: `ReviewList` with sort and filters, media on, totals from the API total, layout decided by whether the store holds demographics; no rating filter on the list. B0: `guarantee`, `certifications` and product evidence replace it and the omission is recorded. | verbatim, dated, variant purchased where stored; one 3-star or lower visible by default at 20 or more. | `reviews_status`, per-star counts, `has_media`. |
| `cross-sell` (conditional: a named relationship, below `reviews`) | two or three items that complete the routine, each with its fit reason. | yes; one catalog `identity` image per item via `lexsis_catalog.get`. Gap: no generation is feasible for identity. | `QuickAdd` per card when the page runs cart v2 and the items need a variant picker, links to the PDPs otherwise; `ProductCarousel` only at four or more items, entry animation off unless the plan names it (N10). | the relationship named ("Complete the routine"); one fit sentence per item. | catalog relationships the merchant confirms. |
| `faq` | five to eight objection-phrased questions, collapsed. | no. | `none`; native `<details>` and `<summary>`. | answers at most 60 words; "Will it feel heavy or cakey?" phrasing. | `reviews_search` on objection topics. |
| `closing-cta` | repeat add-to-cart with the guarantee; last-word quote above. | optional reuse of a verified `in-use` slot; no new job, no generation. | `none`; the CTA anchors to the buy box. | same verb and object as the buy box. | the last-word `review-quote` row. |
